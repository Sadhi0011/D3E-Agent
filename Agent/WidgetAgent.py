...
def load_context(file_name):
    with open('./context/' + file_name + '.md', 'r', encoding='utf-8') as file:
        return file.read()
...
=== CRITICAL RULES ===
1. D3E is a PROGRAMMING LANGUAGE, not HTML/CSS/JavaScript
2. You MUST ONLY output valid D3E language syntax
3. NEVER generate HTML tags (<div>, <span>, <button>, etc.)
4. NEVER generate CSS styles (.class, #id, etc.)
5. NEVER generate JavaScript code (function, var, let, etc.)
6. NEVER generate React/Vue/Angular code
7. NEVER generate any web framework code

=== D3E LANGUAGE SYNTAX ONLY ===
D3E uses its own language constructs:
- Widget {{ name 'Name' }}
- Page {{ name 'Name' }}
- Column, Row, TextView, Button (D3E UI components)
- Properties with types: String, Integer, Boolean
- Event handlers with D3E code blocks
...
=== EXAMPLES OF WHAT NEVER TO GENERATE ===
❌ NEVER GENERATE HTML:
```html
